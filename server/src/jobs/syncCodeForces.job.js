      ...
      while (true) {
        ...
        for (const user of users) {
          try {
            ...
            const lastSync = user.codeforcesSynced.lastSubmissionSyncAt
              ? user.codeforcesSynced.lastSubmissionSyncAt.getTime() / 1000
              : 0;
            const newAccepted = accepted.filter((s) => s.creationTime > lastSync);
            const seen = new Set();
            ...
            if (hasNewSolve) {
              await userModel.findByIdAndUpdate(user._id, {
                "codeforcesSynced.lastSubmissionSyncAt": new Date(),
              });
            }
          } catch (error) {
            console.error(`Failed to sync ${user.codeforcesHandle}:`, error.message);
            continue;
          }
