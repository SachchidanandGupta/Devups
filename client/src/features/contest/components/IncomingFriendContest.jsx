...
const IncomingFriendContest = ({ contest }) => {
  const { creator, contestName, endTime } = contest || {};
  console.log(creator);
  return (
  ...
  <div className="grid grid-cols-5 gap-1 mt-1">
    <button className="col-span-4 bg-accent px-1 py-2 border border-border font-bold text-text-muted cursor-pointer active:scale-95 hover:bg-text-primary hover:border-border transition-colors truncate">
      Accept
    </button>
    <button className="col-span-1 bg-surface border border-border cursor-pointer hover:border-danger hover:text-danger flex items-center justify-center transition-colors">
      x
    </button>
  </div>
</div>
